...
async function run() {
  try {
    ...
    app.get("/users/buyer", async (req, res) => {
      const query = {};
      //  const buyers = await bookingsCollection
      //    .find(query);
      const buyers = await usersCollection.find(query).toArray();
      console.log(buyers.role);
      if (buyers?.role === "Buyer") {
        console.log(buyers);
        res.send(buyers);
      };
    });
    ...
    app.patch("/sellers/:email",  async (req, res) => {
      const email = req.params.email;
      const status = req.body.status;
     ...
     app.delete("/sellers/:email", verifyJWT, async (req, res) => {
       const email = req.params.email;
